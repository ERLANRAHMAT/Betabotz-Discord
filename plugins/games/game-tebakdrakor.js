...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    const gameEmbed = new EmbedBuilder()
      .setColor(0x00AEEF) // Warna biru muda
      .setTitle("📺 Tebak Judul Drama Korea!")
      .setDescription(`**Petunjuk:**\n> ${question.deskripsi}`)
      .setImage(question.img) // Menampilkan gambar adegan dari API
      .addFields(
    ...
    collector.on('collect', async msg => {
        ...
        if (userGuess === session.answer.toLowerCase()) {
            await msg.reply(`✅ **Daebak!** Jawaban yang benar adalah **${session.answer}**.\nKamu pencinta drakor sejati! +${rewardPoint.toLocaleString()} Poin untukmu!`);
