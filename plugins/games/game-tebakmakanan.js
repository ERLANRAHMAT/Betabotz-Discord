...
async function fetchQuestion() {
    try {
        const response = await fetch(`https://api.betabotz.eu.org/api/game/tebakmakanan?apikey=${config.apikey_lann}`);
        if (!response.ok) throw new Error(`API returned status ${response.status}`);
        const data = await response.json();
        // Memilih satu soal secara acak dari array
        return data[Math.floor(Math.random() * data.length)];
    } catch (error) {
        console.error("[TEBAK MAKANAN] Gagal mengambil data dari API:", error);
        return null;
    }
}
...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (gameSessions.has(channelId)) {
        return message.reply("❗ Masih ada sesi Tebak Makanan yang sedang berlangsung di channel ini.");
    }
    ...
    const gameEmbed = new EmbedBuilder()
      ...
      .setTitle("🍔 Tebak Nama Makanan Ini!")
      .setDescription(`**Petunjuk:**\n> ${question.deskripsi}`)
      .setImage(question.img) // Menampilkan gambar makanan dari API
      .addFields(
          { name: "Waktu", value: `⏳ ${gameTimeout / 1000} detik`, inline: true },
          { name: "Hadiah", value: `💰 ${rewardPoint.toLocaleString()}`, inline: true }
      )
      .setFooter({ text: "Ketik jawabanmu, !tebma untuk bantuan, atau !suren untuk menyerah." });
    ...
    collector.on('collect', async msg => {
        ...
        if (userGuess === session.answer.toLowerCase()) {
            await msg.reply(`✅ **Mantap!** Jawabannya adalah **${session.answer}**.\nPerut jadi keroncongan! Kamu dapat +${rewardPoint.toLocaleString()} Poin!`);
            collector.stop('correct');
        } else if (similarity(userGuess, session.answer.toLowerCase()) >= similarityThreshold) {
            msg.reply("❗ **Hampir benar!** Coba lagi, namanya mirip.");
        } else {
            msg.react('❌').catch(() => {});
        }
    });
