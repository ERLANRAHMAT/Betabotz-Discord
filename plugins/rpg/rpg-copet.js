  ...
  async execute(message, args, client) {
    ...
    if (activeMissions.has(authorId)) {
      return message.reply(
        "❗ Anda sedang menyelesaikan misi lain. Tunggu sampai selesai!",
      );
    }
    ...
    try {
      ...
      if (currentTime - lastCopet < cooldown) {
        ...
        return message.reply(
          `🚓 Polisi masih berpatroli, jangan gegabah!\nKembali lagi dalam **${clockString(remainingTime)}**.`,
        );
      }
      if (userData.stamina < staminaCost) {
        return message.reply(
          `❗ Stamina tidak cukup. Kamu butuh setidaknya **${staminaCost}** stamina untuk mencopet.`,
        );
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0x34495e)
        .setTitle("💀 Misi Copet Dimulai");
      const missionMessage = await message.reply({
        embeds: [embed.setDescription("🔍 Mengincar target di keramaian...")],
      });

      await delay(4000);
      await missionMessage.edit({
        embeds: [embed.setDescription("➕ Memulai aksi...")],
      });
      await delay(4000);
      await missionMessage.edit({
        embeds: [embed.setDescription("👜 Merampas tas target...")],
      });
      await delay(4000);
      await missionMessage.edit({
        embeds: [
          embed.setDescription("🏃‍♂️ Berhasil kabur! Menghitung hasil..."),
        ],
      });
      await delay(2000);
      ...
      if (userData.warn >= 5) {
        userData.jail = {
          status: true,
          reason: "Terlalu banyak melakukan copet!",
          until: Date.now() + 24 * 60 * 60 * 1000,
        };
        userData.warn = 0;
        await api.updateUser(authorId, userData);
        await missionMessage.edit(
          "⛓️ Kamu tertangkap polisi karena terlalu sering mencopet! Kamu masuk penjara selama 1 hari.",
        );
      ...
      const resultEmbed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: "💰 Uang Didapat",
            value: `+${randomMoney.toLocaleString("id-ID")}`,
            inline: true,
          },
          {
            name: "✨ XP Didapat",
            value: `+${randomExp.toLocaleString("id-ID")}`,
            inline: true,
          },
      ...
      message.reply(
        `❌ Terjadi kesalahan saat menjalankan misi: ${error.message}`,
      );
    } finally {
      activeMissions.delete(authorId);
    }
