...
const API_BASE = "/features/waifus";
...
async function presentWaifu(originalMessage, author, waifuData, remaining) {
  ...
  if (!details)
    return originalMessage.edit({
      content: `❌ Terjadi kesalahan: Data karakter tidak ditemukan.`,
    });

  const galleryImages = images.slice(0, 10);
  if (galleryImages.length === 0)
    return originalMessage.edit({
      content: `❌ Karakter **${details.nama}** ditemukan, tetapi tidak ada gambar.`,
    });
  ...
  const createEmbed = (index) => {
    const description =
      details.anime === "Discovered"
        ? `**${details.nama}**`
        : `**${details.nama}** dari *${details.anime}*`;
    const embed = new EmbedBuilder()
      .setColor(0xffb6c1)
      .setTitle(details.nama)
      .setURL(
        `https://www.google.com/search?q=${encodeURIComponent(details.nama + " " + details.anime)}`,
      )
      .setDescription(description)
      .setImage(galleryImages[index])
      .setFooter({
        text: `Sisa jatah harianmu: ${remaining}/${waifuLimiter.MAX_USES}`,
      });
    if (rank)
      embed.addFields({
        name: "🏆 Peringkat",
    ...
    if (galleryImages.length > 1)
      embed.addFields({
        name: "🖼️ Gambar",
        value: `${index + 1} / ${galleryImages.length}`,
        inline: true,
      });
    return embed;
  };
  const createButtons = (index, disabled = false) => {
    return new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder()
        .setCustomId("claim_waifu")
        .setLabel("Klaim")
        .setEmoji("💖")
        .setStyle(ButtonStyle.Success)
        .setDisabled(disabled),
      new ButtonBuilder()
        .setCustomId("skip_waifu")
        .setLabel("Lewati")
      ...
      new ButtonBuilder()
        ...
        .setEmoji("▶️")
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(disabled || index >= galleryImages.length - 1),
    );
  };
      ...
      case "skip_waifu":
        ...
        if (i.customId === "claim_waifu") {
          try {
            ...
            await api.updateUser(author.id, userData);
            // --- END CEK UANG ---

            const chosenImageUrl = galleryImages[currentIndex];
            const res = await postWaifuData("/claim", {
              userId: author.id,
              waifuid: details.waifuid,
            ...
            await response.edit({
              content: `❌ Gagal klaim: ${err.message}`,
              embeds: [],
              components: [],
            });
          }
    ...
    if (reason === "time")
      response
        .edit({
          content: "⌛ Waktu habis!",
          embeds: [],
          components: [createButtons(currentIndex, true)],
        })
        .catch(() => {});
  });
}
...
async function handleGacha(message, args, client) {
  ...
  if (!limit.isAllowed)
    return message.reply(
      `❗ Jatah harianmu habis. Sisa: **${limit.remaining}/${waifuLimiter.MAX_USES}**`,
    );
  ...
  try {
    ...
    await processingMsg.edit(`❌ Gagal meroll waifu: ${e.message}`);
  }
}
...
async function handleSearch(message, args, client) {
  ...
  if (!name)
    return message.reply(
      "Masukkan nama waifu. Contoh: `!waifu search Minato Aqua`",
    );
  ...
  const processingMsg = await message.reply(
    `🔍 Mencari waifu bernama **${name}**...`,
  );
  try {
    const res = await waifuApi.get(`${API_BASE}/search`, {
      params: { nama: name },
    });
    ...
    if (res.data && res.data.status) {
      ...
      throw new Error(
        res.data.message || "API tidak memberikan respons yang valid.",
      );
    }
  } catch (e) {
    const errorMessage =
...
async function handleMyWaifu(message, args, client) {
  const processingMsg = await message.reply(
    "💍 Memeriksa status pernikahanmu...",
  );
  const userId = message.author.id;
  try {
    const marriage = await getWaifuData("/mymarriage", { userId });
    const waifu = marriage.waifu;
    const marriedAt = new Date(marriage.marriageDate).toLocaleDateString(
      "id-ID",
    ...
    const marriedImageBuffer = await createMarriedImage(
      userAvatar,
      waifu.imgurl,
    );

    if (marriedImageBuffer) {
      const attachment = new AttachmentBuilder(marriedImageBuffer, {
        name: "waifu-married.png",
      });
    ...
    return processingMsg.edit(
      "💔 Kamu belum menikah. Gunakan `!waifu marry` untuk menikahi salah satu waifumu yang sudah di-klaim!",
    );
  }
}
...
async function handleUnclaim(message, args, client) {
  ...
  try {
    ...
    if (!claims || claims.length === 0)
      return processingMsg.edit("❌ Kamu tidak punya waifu untuk dilepaskan.");
    ...
    const response = await processingMsg.edit({
      content: "Siapa yang akan kamu lepaskan dari koleksimu?",
      components: [row],
    });
    ...
    collector.on("collect", async (i) => {
      ...
      const unclaimedWaifu = claims.find(
        (c) => c.waifu.waifuid.toString() === waifuIdToUnclaim,
      );
      const waifuName = unclaimedWaifu
        ? unclaimedWaifu.waifu.nama
        : "Waifu tersebut";

      try {
        await deleteWaifuData("/unclaim", {
          userId,
          waifuid: waifuIdToUnclaim,
        });
        await response.edit({
        ...
        await response.edit({
          content: `❌ Gagal melepaskan: ${e.message}`,
          components: [],
        });
      }
      collector.stop();
...
async function handleMyList(message, args, client) {
  const processingMsg = await message.reply(
    "💖 Mengambil daftar waifu yang kamu klaim...",
  );
  const userId = message.author.id;
  try {
    const claims = await getWaifuData("/myclaim", { userId });
    if (!claims || claims.length === 0)
      return processingMsg.edit(
        "😢 Kamu belum memiliki waifu yang di-klaim. Gunakan `!waifu roll`!",
      );
    ...
    const selectMenu = new StringSelectMenuBuilder()
      .setCustomId("mywaifu_list_select")
      .setPlaceholder("Pilih waifu untuk dilihat detailnya...")
      .addOptions(options);
    const response = await processingMsg.edit({
      content: `Kamu memiliki **${claims.length}** waifu yang di-klaim.`,
...
async function handleMarry(message, args, client) {
  ...
  try {
    ...
    if (!claims || claims.length === 0)
      return processingMsg.edit(
        "❌ Kamu harus meng-klaim waifu terlebih dahulu sebelum menikah.",
      );
    const options = claims.map((c) => ({
      label: c.waifu.nama.substring(0, 100),
      description: `Bisa dinikahi setelah: ${new Date(c.claimDetails.marriedAt).toLocaleDateString("id-ID")}`,
      value: c.waifu.waifuid.toString(),
    }));
    const row = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        .setCustomId("marry_select")
        .setPlaceholder("Pilih waifu yang akan dinikahi...")
        .addOptions(options),
    );
    const response = await processingMsg.edit({
      content: "Pilih waifu dari daftar klaimmu yang ingin kamu nikahi:",
      components: [row],
    });
    const collector = response.createMessageComponentCollector({
      filter: (i) => i.user.id === userId,
    ...
    collector.on("collect", async (i) => {
      ...
      try {
        ...
        await response.edit({
          content: `❌ Gagal menikah: ${e.message}`,
          components: [],
        });
      }
      collector.stop();
...
async function handleUnmarry(message, args, client) {
  ...
  try {
    ...
    const confirmEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `Apakah kamu benar-benar yakin ingin bercerai dengan **${waifuName}**?\n\nTindakan ini tidak bisa dibatalkan.`,
      );

    const buttons = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("unmarry_confirm")
        .setLabel("Ya, Ceraikan")
        .setStyle(ButtonStyle.Danger),
      new ButtonBuilder()
        .setCustomId("unmarry_cancel")
        .setLabel("Batal")
        .setStyle(ButtonStyle.Secondary),
    );
    ...
    collector.on("collect", async (i) => {
      ...
      if (i.customId === "unmarry_confirm") {
        try {
          ...
          await response.edit({
            content: `❌ Gagal bercerai: ${e.message}`,
            embeds: [],
            components: [],
          });
        }
...
async function handleRank(message, args, client) {
  ...
  try {
    ...
    const rankList = ranks
      ...
      .map(
        (w, i) =>
          `**${i + 1}.** **${w.nama}** - Diklaim oleh **${w.claimedBy.length}** orang`,
      )
      .join("\n");
    const embed = new EmbedBuilder()
      .setColor(0xffd700)
      .setTitle("🏆 Top 10 Waifu Terpopuler")
      .setDescription(rankList);
    await processingMsg.edit({ content: null, embeds: [embed] });
  } catch (e) {
    await processingMsg.edit(`❌ Gagal mengambil ranking: ${e.message}`);
  }
}
...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    switch (subCommand) {
      case "search":
        return handleSearch(message, restArgs, client);
      case "roll":
      case "gacha":
      ...
      case "unclaim":
        return handleUnclaim(message, restArgs, client);
      case "marry":
      case "nikah":
        return handleMarry(message, restArgs, client);
      case "unmarry":
      case "cerai":
        return handleUnmarry(message, restArgs, client);
      case "rank":
      case "top":
        return handleRank(message, restArgs, client);
      default:
        ...
        const helpEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `**Sisa Jatah Harian:** ${limit.remaining}/${waifuLimiter.MAX_USES}`,
          )
          .addFields(
            { name: "`!waifu roll`", value: "Mencari waifu acak." },
            {
              name: "`!waifu search <nama>`",
              value: "Mencari waifu spesifik.",
            },
            {
              name: "`!waifu my`",
              value: "Melihat waifu yang sudah kamu nikahi.",
            }, // [DIUBAH]
            {
              name: "`!waifu list`",
              value: "Melihat daftar waifu yang kamu klaim.",
            }, // [BARU]
            {
              name: "`!waifu unclaim`",
              value: "Melepaskan salah satu waifu yang kamu klaim.",
            },
            {
              name: "`!waifu marry`",
              value: "Menikahi salah satu waifu yang kamu klaim.",
            },
            {
              name: "`!waifu unmarry`",
              value: "Menceraikan waifu yang sudah dinikahi.",
            },
            { name: "`!waifu rank`", value: "Melihat papan peringkat." },
            { name: "`!claim`", value: "Claim waifu Virtunix langsung." },
          );
